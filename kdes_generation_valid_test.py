...
def get_ats(
    ...
    layer_outputs1 = temp_model.predict(dataset[:len(dataset)//2], batch_size=batch_size, verbose=1)
    layer_outputs2 = temp_model.predict(dataset[len(dataset)//2:], batch_size=batch_size, verbose=1)
    layer_outputs = np.concatenate([layer_outputs1,layer_outputs2],axis=0)
...
def _get_kdes(train_ats, class_matrix, args):
    ...
    for label in tqdm(range(args.num_classes), desc="kde"):
        ...
        #     removed_cols2 = np.where(variances < mid)[0]
        #     refined_ats2 = np.transpose(train_ats[class_matrix[label]])
        #     refined_ats2 = np.delete(refined_ats2, removed_cols2, axis=0)

        #     try:
        #         kde_label = gaussian_kde(refined_ats2)
        #     except:
        #         print("In %s loop: An error occurs in bisection method" % i)
        #         continue

        #     outputs = kde_label(refined_ats2)
        #     max_kde2 = np.max(outputs)
        #     min_kde2 = np.min(outputs)
        #     print("min_kde: %s" % min_kde2)
        #     print("max_kde: %s" % max_kde2)

        #     print("for loop -- max&min:",max_kde2,min_kde2)
        #     if math.isinf(max_kde2) or math.isinf(min_kde2):
        #         pre_var_threshold = mid
        #         print("Bisection: value inf")
        #     else:
        #         args.var_threshold = mid
        #         kdes[label] = kde_label
        #         refined_ats = refined_ats2
        #         removed_cols = removed_cols2
                
            
        #print("aft inf issue ",args.var_threshold)
...
def fetch_kdes(model, x_train, y_train, layer_names, args):
    ...
    for layer_name in layer_names:
        ...
        #         os.remove(kdes_file)
        #     #args.var_threshold = 2e-1 comment this line to see the error and set automatic way to find
        ...
        if os.path.exists(kdes_file):
            ...
            if (flag_all or flag_inf) and args.var_threshold != 0:
                # if (idx == 1): pre_var_threshold = args.var_threshold
